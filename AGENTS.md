...
6. **Contexto del proyecto** Este repo es un subconjunto del ecosistema AJE real. `aje_docs_simulacion/01_Docs_Oficiales/` es documentacion de referencia del sistema Big Magic ERP (el core real de AJE con integraciones SOAP/REST a SAP, FullStep, Avail, Salesforce, etc) — NO se modifica, solo se consulta para contexto,evitando consultar los '.docx' '.pdf' '.pptx' ,priorizar los '.md' . El trabajo de desarrollo va exclusivamente en `Pruebas/Production/` (modulo Odoo para Mexico). `tutorials/` son ejercicios de aprendizaje.
...
8. **descripciones detalladas de las funciones** La columna 'Qué hace'  (data_para_gente/bm_ctl_produccion_descripciones.md) de las funciones (program # : NUMERO) aun no validadadas/analizadas  y que aun no poseen su modelo (los .pys) son meramente referenciales , no son fuente de verdad para las consultas que el agente considere necesarias.